...
@Controller
@RequestMapping(value="/sfjldiabetes")
public class SfjlDiabetesController extends BaseController {
	String menuUrl = "sfjldiabetes/list.do"; //菜单地址(权限用)
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			page.setPd(pd);
			List<PageData>	list = sfjlDiabetesService.list(page);  //列表
			mv.setViewName("gp/sfjl/sfjldiabetes/sfjldiabetes_list");
	...
	public ModelAndView goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = sfjlDiabetesService.findById(pd);					 //根据ID读取
			...
			pd.put("EnumMzzdwhCRB", EnumMzzdwhCRB.toMap());//用药干预，与“门诊诊断维护，是否传染病”同一枚举
			mv.setViewName("gp/sfjl/sfjldiabetes/sfjldiabetes_edit");
