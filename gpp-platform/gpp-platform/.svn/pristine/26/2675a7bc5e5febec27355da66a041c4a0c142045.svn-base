...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="curedisease/list.do?OGR_CODE=${ORG_CODE}" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;width: 120px;">
									<select class="pos-rel" name="queryDiseaseType" id="queryDiseaseType" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
										<option value="" selected="selected">&nbsp;全部类别</option>
										<c:forEach items="${pd.enumDiseaseType}" var="item">
											<c:choose>
												<c:when test="${pd.queryDiseaseType == item.value}">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" style="width:30%;">疾病编码</th>
									<th class="center" style="width:30%;">疾病名称</th>
									<th class="center" style="width:20%;">疾病类别</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.CUREDISEASE_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.DISEASE_CODE}</td>
											<td class='center'>${var.DISEASE_NAME}</td>
											<td class='center'>${var.DISEASE_TYPE}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.CUREDISEASE_ID}');">
														 编辑
													 </a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" title="删除"  onclick="del('${var.CUREDISEASE_ID}');">
														删除
													 </a>
													</c:if>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.CUREDISEASE_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		function reset(){
			$("#keywords").val("");
			$("#queryDiseaseType").find("option:selected").attr("selected",false);
			$("#queryDiseaseType").find("option[text='']").attr("selected",true);
		}
...
//			 top.jzts();
			 ...
			 if(jQuery.isEmptyObject(nodes) || nodes[0].name=="全部")
			 {
				...
				return;
				 ...
				 diag.Drag=true;
				 diag.Title ="新增";
				 diag.URL = '<%=basePath%>curedisease/goAddDis.do?ORG_CODE='+ORG_CODE;
				 diag.Width = 900;
				 diag.Height = 575;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>curedisease/delete.do?CUREDISEASE_ID="+Id+"&tm="+new Date().getTime();
...
//			 top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>curedisease/goEdit.do?CUREDISEASE_ID='+Id;
			 diag.Width = 400;
			 diag.Height = 250;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					...
					if(result) {
						...
						if(str==''){
							...
							if(msg == '确定要删除选中的数据吗?'){
								...
								$.ajax({
									type: "POST",
									url: '<%=basePath%>curedisease/deleteAll.do?tm='+new Date().getTime(),
									data: {DATA_IDS:str},
									dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>curedisease/excel.do';
		}
	</script>
