		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="shape/list.do?USER_AGENT_ID=${pd.USER_AGENT_ID }" method="post" name="shapeForm" id="shapeForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">体重(kg)</th>
									<th class="center">身高(cm)</th>
									<th class="center">腰围(cm)</th>	
									<th class="center">臀围(cm)</th>
									<th class="center">BIM</th>
									<th class="center">腰臀比%</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">${element.WEIGHT }</td>
											<td class="center">
								                ${element.HEIGHT } 
											</td>
											<td class="center">${element.WAIST  }</td>
											<td class="center">
												${element.HIPLINE   }
											</td>
											<td class="center">${element.BMI  }</td>
											<td class="center">
												${element.WHR  }
											</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		//新增
		  function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增体型记录";
			 diag.URL = '<%=basePath%>shape/add.do?USER_AGENT_ID=${pd.USER_AGENT_ID }';
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改体型记录";
			 diag.URL = '<%=basePath%>shape/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function del(id,msg){
			bootbox.confirm("确定要删除此体型记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>shape/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>shape/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
