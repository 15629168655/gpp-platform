						...
						<div class="col-xs-12">
							...
							<table style="margin-top:5px;">
								...
								<td style="vertical-align:top;padding-left:2px;"> 
								 	<select class="chosen-select form-control" name="SCREENING_TYPE" id="SCREENING_TYPE" style="vertical-align:top;width: 180px;">
										<option value="">--请选择筛查类型--</option>
										<c:forEach items="${SCREENTYPE}" var="item">
                                            <option value="${item.value.ADDITIONAL}" <c:if test="${pd.SCREENING_TYPE == item.value.ADDITIONAL}"> selected="selected" </c:if> >${item.value.NAME}</option>
                                        </c:forEach>
									</select>
								</td>
								<td style="vertical-align:top;padding-left:2px;"> 
					...
					<table id="simple-table" class="table table-bordered " style="margin-top:5px;width:100%" >	
							<thead>
								<tr>
									...
									<th class="center">筛查人数</th>
									<th class="center">疾病诊断</th>	
								</tr>
							</thead>
							<tbody>
...
<script src="static/ace/js/ace/ace.widget-box.js"></script>
	<script type="text/javascript">
		...
		function initData() {
			var url='<%=basePath%>quesResultStatistic/diseaseStatisticData.do';
			$.post(url,{},function(data){
				...
				for(var i=0;i<data.xAxis.length;i++){
					html += '<option value="'+data.jbid[i]+'">'+data.xAxis[i]+'</option>';
				}
				$("#food").append(html);
				//多选下拉框
	...
	function searchs() {
		...
		if(type=='0'){
			...
			$.post(url,param,function(data){
				if(data.msg =='00'){
					...
					//	var i =0; 
						item = "<tr><td width='500px' align='center'>"+data.xAxis[i]+"</td><td width='400px' align='center'>"+data.sz[i]+"</td><td width='320px' align='center' ><a class='btn btn-xs btn-success' title='诊断' onclick=\"disDiagnosis('" + data.jbid[i] + "')\">"+ '诊断' +"</a></td></tr>"; 
						//i++;
						$('.table').append(item); 
					}
					show();
		...
		function toExcel(){
			...
			NND=encodeURI(DISEASECODE_ID); //中文参数转码
			window.location.href='<%=basePath%>quesResultStatistic/excel.do?APPLYSTATUS='+APPLYSTATUS+'&applyDateStart='+applyDateStart+'&applyDateEnd='+applyDateEnd+'&DISEASECODE_ID='+DISEASECODE_ID;
		}
		...
		//疾病诊断
		function disDiagnosis(id) {
			top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="疾病诊断";
			 diag.URL = '<%=basePath%>quesResultStatistic/disDiagnosis.do?DISEASECODE_ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
