			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="bookingAccredit/yhrylist.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.PROVIDER_NAME}" id = "${var.ID}" class="ace" /><span class="lbl"></span></label>
											</td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<a class="btn btn-sm btn-success" onclick="makeAll('确定给选中的医生授权吗?');" title="批量授权" >批量授权</a>
								</td>
								<td style="vertical-align:top;">
				...
				if(result) {
					...
					for(var i=0;i < document.getElementsByName('ids').length;i++){
					  if(document.getElementsByName('ids')[i].checked){
					  	if(strid=='') strid += document.getElementsByName('ids')[i].id;
					  	else strid += ',' + document.getElementsByName('ids')[i].id;
					  	if(strname=='') strname += document.getElementsByName('ids')[i].value;
					  	else strname += ',' + document.getElementsByName('ids')[i].value;
					  }
					}
					if(strid==''){
						bootbox.dialog({
						...
						if(msg == '确定给选中的医生授权吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>bookingAccredit/save.do?tm='+new Date().getTime(),
						    	data: {IDS:strid,NAMES:strname},
								dataType:'json',
