...
$(function (){
    ...
    if(requestParams['loginpath'] !=undefined && requestParams['loginpath'] != ""){
        loginpath = requestParams['loginpath'];
    }
});
...
function getHeadMsg(){
    $.ajax({
		type: "POST",
		url: locat+'/head/getList.do?tm='+new Date().getTime()+'&loginpath='+loginpath,
    	data: encodeURI(""),
		dataType:'json',
...
//下线
function goOut(msg){
	window.location.href=locat+"/logout.do?msg="+msg+"&loginpath="+loginpath;
}
...
function editUserH() {
    ...
    if (loginpath == 'adminLogin') {
        ...
        diag.Title = "会员资料";
        diag.URL = locat+'/happuser/goEditU.do?USER_ID=' + USER_ID+'&editMode=self';
        diag.Width = 600;
        diag.Height = 419;
    }
