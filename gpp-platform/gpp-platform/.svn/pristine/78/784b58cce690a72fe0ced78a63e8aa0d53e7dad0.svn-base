		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="sugar/list.do?USER_AGENT_ID=${pd.USER_AGENT_ID }" method="post" name="sugarForm" id="sugarForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">测试时段</th>
									<th class="center">血糖值(mmol/L)</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">
												<c:forEach items="${pd.TIMEENUM}" var="s">
                                                    <c:if test="${s.key == element.TIME_CLASS}">${s.value}</c:if>
                                                </c:forEach>
											<td class="center">
								                ${element.SUGAR_VALUE } 
											</td>
	...
	<script type="text/javascript">
		...
		//新增
		  function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增血糖记录";
			 diag.URL = '<%=basePath%>sugar/add.do?USER_AGENT_ID=${pd.USER_AGENT_ID }';
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改血糖记录";
			 diag.URL = '<%=basePath%>sugar/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		//删除
		function del(id,msg){
			bootbox.confirm("确定要删除此血糖记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>sugar/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>sugar/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
