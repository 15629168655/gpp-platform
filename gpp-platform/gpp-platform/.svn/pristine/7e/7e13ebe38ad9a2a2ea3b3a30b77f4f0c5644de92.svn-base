		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="indexCombination/list.do?" method="post" name="fatForm" id="fatForm">
...
<script type="text/javascript" src="static/js/jquery.tips.js"></script>
	...
	<script type="text/javascript">
	  ...
	  function add(){
		 top.jzts();
		 top.siMenu('z71','lm70','新增指标组合','indexCombination/addIndex.do');
		 
	}	
	//修改
	function edit(id){
		 top.jzts();
	     top.siMenu('z71','lm70','修改指标组合','indexCombination/editIndex.do?INDEX_COMBINATION_ID='+id+'&ID='+id);
	} 
		/* function add(){
            //top.jzts();
		...
		function del(id,msg){
			bootbox.confirm("确定要该指标模板吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>indexCombination/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>indexCombination/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
