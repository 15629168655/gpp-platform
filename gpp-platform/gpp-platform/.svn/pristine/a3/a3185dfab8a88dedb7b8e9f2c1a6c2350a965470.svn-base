...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var setting = {
            ...
            ,async: {
                enable: true,
                url:"<%=basePath%>/district/disTree.do",
                autoParam: ["id", "name"]
            },callback:{
    ...
    function save()
    {
        ...
        {
            bootbox.alert("请选择区划", function() {
                return;
            });
        }
        var iframe = $("#page_z64",parent[0].document.body);
        $("#treeFrame",iframe.contents()).contents().find("#REGIONCODE").val(nodes[0].id);
        $("#treeFrame",iframe.contents()).contents().find("#REGIONVALUE").val(nodes[0].name);
        top.Dialog.close();
    }
