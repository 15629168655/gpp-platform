			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="zzKnowledgeBase/list.do" method="post" name="userForm" id="userForm">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<input class="nav-search-input" autocomplete="off" id="keywords" type="text" name="keywords" value="${pd.keywords }" placeholder="根据症状名称搜索"/>
									</div>
								</td>
							 	<c:if test="${QX.cha == 1 }">
								...
								<c:when test="${not empty list}">
									...
									<c:forEach items="${list }" var="list" >
										<tr>
											...
											<td class="center">${list.CLASS_NAME}</td>
											<td class="center">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增症状信息";
			 diag.URL = '<%=basePath%>zzKnowledgeBase/toAddPage.do';
			 diag.Width = 700;
			 diag.Height = 500;
		...
		function editUser(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改疾病信息";
			 diag.URL = '<%=basePath%>zzKnowledgeBase/toEditPage.do?id='+id;
			 diag.Width = 700;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delObj(id,name){
			bootbox.confirm("确定要删除["+name+"]吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>zzKnowledgeBase/delObject.do?id="+id;
					$.get(url,
						function(data){
							nextPage(${page.currentPage});
						});
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>zzKnowledgeBase/deleteAll.do',
						    	data: {ids:str},
								dataType:'json',
