			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="blfyypgl/list.do" method="post" name="userForm" id="userForm">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:2px;"><input class="span10 date-picker" name="GMT_START" id="GMT_START"  value="${pd.GMT_START}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="生成时间起" title="生成时间起"/></td>
								<td style="padding-left:2px;"><input class="span10 date-picker" name="GMT_END" id="GMT_END"  value="${pd.GMT_END}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="生成时间止" title="生成时间止"/></td>
								<td style="vertical-align:top;padding-left:2px;"> 
								 	<select class="pos-rel" name="category" id="category" data-placeholder="类别" style="vertical-align:top;width: 109px;">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList }" var="varList" varStatus="vs">
										<tr>
											...
											<td class="center">${varList.DRUG_NAME }</td>
											<td class="center">${varList.MZJZLSH }</td>
											<td class="center">${varList.PATIENT_NAME }</td>
											<td class='center'>
                                                <c:forEach items="${pd.enumBlfyypglLB}" var="s">
                                                    <c:if test="${s.key == varList.CATEGORY}">${s.value}</c:if>
                                                </c:forEach>
                                            </td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${varList.ID }','${varList.DRUG_NAME }');">
														删除
													</a>
													</c:if>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${varList.ID }','${varList.DRUG_NAME }');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function reset(){
			...
			$("#GMT_START").val("");
			$("#GMT_END").val("");
			$("#category").find("option:selected").attr("selected",false);
			$("#category").find("option[text='']").attr("selected",true);
		}
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增信息";
			 diag.URL = '<%=basePath%>blfyypgl/goAdd.do';
			 diag.Width = 500;
			 diag.Height = 350;
		...
		function editUser(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改信息";
			 diag.URL = '<%=basePath%>blfyypgl/goEdit.do?id='+id;
			 diag.Width = 500;
			 diag.Height = 350;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>blfyypgl/del.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>blfyypgl/deleteAll.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
