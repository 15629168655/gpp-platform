...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<c:if test="${pd.TYPE != '2' }">
											<td class='center'><a onclick="viewUser1('${var.TO_USERID}')" style="cursor:pointer;">${var.TO_USERNAME}</a></td>
											<td class='center'><a onclick="viewUser1('${var.FROM_USERID}')" style="cursor:pointer;">${var.FROM_USERNAME}</a></td>
											</c:if>
											<c:if test="${pd.TYPE == '2' }">
											<td class='center'><a onclick="viewUser1('${var.FROM_USERID}')" style="cursor:pointer;">${var.FROM_USERNAME}</a></td>
											<td class='center'><a onclick="viewUser1('${var.TO_USERID}')" style="cursor:pointer;">${var.TO_USERNAME}</a></td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.FHSMS == 1 }"><a title="批量发送站内信" class="btn btn-mini btn-info" onclick="sendFhsms1();"><i class="ace-icon fa fa-envelope-o bigger-120"></i></a></c:if>
									<c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		function sendFhsms1(){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>fhsms/goAdd1.do';
			 diag.Width = 660;
			 diag.Height = 444;
			 diag.CancelEvent = function(){ //关闭事件
				 top.jzts();
		...
		//查看用户
		function viewUser1(USERID){
			 
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = '<%=basePath%>happuser/goView.do?USER_ID='+USERID;
			 diag.Width = 469;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
