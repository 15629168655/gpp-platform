        	...
        	if(Tools.checkKey("KEYWORDS", pd.getString("FKEY"))){	//检验请求key值是否合法
	           if(AppUtil.checkParam("appSaveSFJL", pd)){	//检查参数
	       		...
	       		if(pd.containsKey("JOB_ID")) {
		       		...
		       		PageData pdJob = taskService.findById(pd);
		       		if(pdJob.get("JOB_TYPE").toString().equals(Integer.toString(EnumJobType.TREATMENT.getCode()))) {
		       			pd.put("CASE_ID", pdJob.get("BUINESS_ID"));
