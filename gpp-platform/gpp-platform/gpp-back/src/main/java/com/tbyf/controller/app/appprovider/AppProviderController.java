...
public class AppProviderController  extends BaseController {
    ...
    /**
     * app根据当前医生机构查询医护人员接口 
     * 如果是医院医生搜索当前医院的医生和下级机构的团队医生，如果是社区医生，则搜索社区医生所在团队的医生
     * 
     * @param
     * @return
     */ 
    @RequestMapping(value="/getProviderOperTMember",method=RequestMethod.GET)
    @ResponseBody
    public Object queryPageOperTMember()
    {
    	logBefore(logger, "app通过过滤条件（医生机构编码）对医护人员（HM_PROVIDER）分页查询接口");
    	Map<String,Object> map = new HashMap<String,Object>();
    	PageData pd = new PageData();
    	pd = this.getPageData(); 
    	String result = "00";
    	...
    	try{	
    		if(Tools.checkKey("ORG_CODE", pd.getString("FKEY"))){	//检验请求key值是否合法
    			if((Tools.notEmpty(pd.getString("ORG_CODE")))){	//检查参数
    				//如果不传分页参数，给默认值。
    				int pageSize=1;//页码
    				int pageCount=10;//页数
    				if(null !=pd.get("PAGESIZE") && null !=pd.get("PAGECOUNT") ){
    				...
    				List<PageData> list=providerService.getProviderOperTMember(pd);
    				map.put("pd", list);
    				result = "01";
    				message =ResultMessageUtil.MESSAGE_1;
    			}else {
     ...
     * @return
     */ 
    @RequestMapping(value="/getProviderByUserAndCode",method=RequestMethod.GET)
    @ResponseBody
    public Object queryPageByCode()
    {
    	logBefore(logger, "app通过过滤条件（医护人员、机构编码）对医护人员（HM_PROVIDER）分页查询接口");
    	Map<String,Object> map = new HashMap<String,Object>();
    	PageData pd = new PageData();
    	pd = this.getPageData(); 
    	String result = "00";
    	...
    	try{	
    		if(Tools.checkKey("ORG_CODE", pd.getString("FKEY"))){	//检验请求key值是否合法
    			if((AppUtil.checkParam("getProviderByUserAndCode", pd))){	//检查参数
    				//如果不传分页参数，给默认值。
    				int pageSize=1;//页码
    				int pageCount=10;//页数
    				if(null !=pd.get("PAGESIZE") && null !=pd.get("PAGECOUNT") ){
    				...
    				List<PageData> list=providerService.getProviderByCode(pd);
    				map.put("pd", list);
    				result = "01";
    				message =ResultMessageUtil.MESSAGE_1;
    			}else {
