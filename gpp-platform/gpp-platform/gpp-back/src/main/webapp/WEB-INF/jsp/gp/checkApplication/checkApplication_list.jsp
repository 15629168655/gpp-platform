...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="checkApplication/list.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.JZLSH}</td>
											<td class='center'>${var.CFMXH}</td>
											<td class='center'>${var.JCXMMC}</td>
											<td class='center'>${var.JCJG}</td>
											<td class='center'>
												<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${var.JC_TIME}"/>
											</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
				 ...
				 diag.Drag=true;
				 diag.Title ="申请";
				 diag.URL = '<%=basePath%>checkApplication/add.do';
				 diag.Width = 800;
				 diag.Height = 600;
	...
	function edit(id){
		 ...
		 top.jzts();
				 ...
				 diag.Drag=true;
				 diag.Title ="编辑";
				 diag.URL = '<%=basePath%>checkApplication/update.do?ID='+id;
				 diag.Width = 950;
				 diag.Height = 700;
				 diag.CancelEvent = function(){ //关闭事件
					 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id){
				...
				if(result) {
					top.jzts();
					var url = "<%=basePath%>checkApplication/delete.do?ID="+id;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>checkApplication/deleteAll.do',
						    	data: {ids:str},
								dataType:'json',
