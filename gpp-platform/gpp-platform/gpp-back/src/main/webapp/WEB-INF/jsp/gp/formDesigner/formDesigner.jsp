...
<!-- jsp文件头和头部 -->
    ...
    <link href="plugins/Ueditor_Formdesign_Plugins/formdesign/bootstrap/css/site.css" rel="stylesheet" type="text/css" />
	<script type="text/javascript" charset="utf-8" src="plugins/Ueditor_Formdesign_Plugins/formdesign/jquery-1.7.2.min.js"></script>

<script type="text/javascript" charset="utf-8" src="plugins/Ueditor_Formdesign_Plugins/ueditor.config.js"></script>
<script type="text/javascript" charset="utf-8" src="plugins/Ueditor_Formdesign_Plugins/ueditor.all.js"> </script>
<script type="text/javascript" charset="utf-8" src="plugins/Ueditor_Formdesign_Plugins/lang/zh-cn/zh-cn.js"></script>
<script type="text/javascript" charset="utf-8" src="plugins/Ueditor_Formdesign_Plugins/formdesign/formdesign.v4.js"></script>
...
<body class="no-skin">
	<table style="width: 99%;" border="0">
		<tr>
			<td style="width: 25%;" valign="top" bgcolor="#F9F9F9">
			 	<div style="width: 15%;">
...
$(document).ready(function(){
	var ssss = sessionStorage.obj;
	$("#myFormDesign").append(ssss);
	...
	var setting = {
			...
			,async: {
				enable: true,
				url:"<%=basePath%>/form/getTreeData.do",
				autoParam: ["id", "name"]
			},callback:{
	...
	var zn = '[{"id":"0","name":"元素分类","url":"","target":"treeFrame","isParent":true}]';
...
function zTreeOnClick(event,treeId, treeNode) {
	if(!treeNode.isParent){
		$("#element_id").val(treeNode.id);
		element_id = $("#element_id").val();
		formEditor.execCommand(treeNode.type);
	}
};
...
var formDesign = {
    ...
    fnCheckForm : function ( type ) {
        ...
        if(formEditor.hasContents()){
            ...
            parse_form = eval('(' + parse_form + ')');
//            sessionStorage.obj=parse_form.parse;
			var _url= '<%=basePath%>emrform/formDesiginer.do';
			var form_id= sessionStorage.id;
			 $.ajax({
                type: 'POST',
                url : _url,
                dataType : 'json',
                data : {'ID' : form_id,'YSNR':parse_form.parse},
                success : function(data){
					if(data.flag == true) {
						alert('表单保存成功');
						$('li #z134 a', window.parent.parent.document).trigger('click');
					} else {
						alert('表单保存失败');
					}
                }
			...
			<%--  
			 ...
			 diag.Drag=true;
			 diag.Title ="表单信息";
			 diag.URL = '<%=basePath%>form/toAddform.do';
			 diag.Width = 600;
			 diag.Height = 600;
