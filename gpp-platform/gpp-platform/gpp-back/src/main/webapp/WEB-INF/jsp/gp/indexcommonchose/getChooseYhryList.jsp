			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="agreementApply/getPersonData.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<input style="width:100px;" class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="keywords" value="${pd.keywords }" placeholder="请输入姓名" />
									</div>
								</td>
							...
							<c:choose>
								<c:when test="${not empty yhryList}">
									<c:forEach items="${yhryList}" var="yhry" varStatus="vs">
										<tr >
											<td class='agancy' style="width: 30px;">
												<label class="pos-rel"><input type='radio' name='ids' value="${yhry.ID}" id="${yhry.ID}" class="ace" /><span class="lbl"></span></label>
											</td>
		...
		function choose(){
			...
			for(var i=0;i < document.getElementsByName('ids').length;i++){
				if(document.getElementsByName('ids')[i].checked){
				  ...
				  // var iframe = $("#page_z94",parent[0].document.body);
			 	 //  iframe.contents().find("#GEN_PRACTITIONER_ID").val(pid);
				  // iframe.contents().find("#GEN_PRACTITIONER_NAME").val(pname);
				//   top.Dialog.close();
				   
				   var iframe = $("#page_z94",parent[0].document.body);
				   var url="<%=basePath%>agreementApply/idSearch.do";
				   $.post(url,{"ID":pid},
						function(data){
					iframe.contents().find("#GEN_PRACTITIONER_ID").val(data.GEN_PRACTITIONER_ID);
					iframe.contents().find("#GEN_PRACTITIONER_NAME").val(data.GEN_PRACTITIONER_NAME);
					iframe.contents().find("#GEN_PRACTITIONER_PHONE").val(data.GEN_PRACTITIONER_PHONE);
					top.Dialog.close();
