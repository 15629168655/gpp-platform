		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="indexResidentQuse/list.do" method="post" name="fatForm" id="fatForm">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">
								                ${element.QUESTIONNAIRE_RESULT }
											</td>
											<td class="center">
												${element.QUESTIONNAIRE_SUGGESTION }
											</td>
											<td class="center">${element.QUESTIONNAIRE_COMPANY }</td>
											<td class="center">${element.QUESTIONNAIRE_DEPARTMENT }</td>
											<td class="center">${element.QUESTIONNAIRE_PEOPLE  }</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${element.IS_FINISH == 0 }">
														<a class="btn btn-xs btn-info"  title="问卷" onclick="ques('${element.SCREENING_QUESTIONNAIRE_ID }','${element.ID}','${element.RESIDENT_ID}');" >
															问卷
														</a>
													</c:if>
													<c:if test="${element.IS_FINISH == 1 }">
														<a class="btn btn-xs btn-info"  title="详情" onclick="showInfo('${element.SCREENING_QUESTIONNAIRE_ID }','${element.ID}','${element.RESIDENT_ID}');" >
															详情
														</a>
													</c:if>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${element.ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${element.ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
<script type="text/javascript" src="static/js/jquery.tips.js"></script>
	...
	<script type="text/javascript">
	  ...
	  function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="居民问卷新增新增";
		 diag.URL = '<%=basePath%>indexResidentQuse/add.do';
		 diag.Width = 900;
		 diag.Height = 1000;
		...
		//问卷 id居民筛查问卷ID,ques_id居民问卷ID,居民信息ID resident_id
		function ques(id,ques_id,resident_id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="居民问卷";
			 diag.URL = '<%=basePath%>indexResidentQuse/residentQues.do?ID='+id+'&QUES_ID='+ques_id+'&USER_AGENT_ID='+resident_id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		//详情界面
		function showInfo(id,ques_id,resident_id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="居民问卷结果";
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改居民问卷";
			 diag.URL = '<%=basePath%>indexResidentQuse/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function del(id,msg){
			bootbox.confirm("确定要该居民问卷吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>indexResidentQuse/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>indexResidentQuse/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
