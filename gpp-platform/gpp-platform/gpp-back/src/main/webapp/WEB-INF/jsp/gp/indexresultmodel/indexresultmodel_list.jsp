		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="indexResultModel/list.do?INDEX_ID=${pd.INDEX_ID }" method="post" name="fatForm" id="fatForm">
							<table style="margin-top:5px;">
								<tr>
									<td>
									<div class="nav-search">
										<select class="form-control"  name="FIT_SEX" id="FIT_SEX" style="width:80px;">
								...
								<td>
									<div class="nav-search">
										<input name="keywords" id="keywords" value="${pd.keywords }" type="text" placeholder="模板类型" title="模板类型"/>
									</div>
								</td>
								<td>
									<div class="nav-search">
										<input name="MODEL_CONTENT" id="MODEL_CONTENT" value="${pd.MODEL_CONTENT }" type="text" placeholder="模板内容" title="模板内容"/>
									</div>
								</td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">是否呈阳性</th>
									<th class="center">适用的性别</th>
									<th class="center">提示内容</th>	
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">${element.MODEL_CONTENT }</td>
											<td class="center">
								                <c:forEach items="${ISPOSITIVE}" var="s">
								                	<c:if test="${s.key == element.IS_POSITIVE}">${s.value}</c:if>
								                </c:forEach>
											</td>
											<td class="center">
											...
											<td class="center">${element.MODEL_ORDER  }</td>
...
<script type="text/javascript" src="static/js/jquery.tips.js"></script>
	...
	<script type="text/javascript">
	  ...
	  function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="模板结果新增";
		 diag.URL = '<%=basePath%>indexResultModel/add.do?INDEX_ID=${pd.INDEX_ID}';
		 diag.Width = 900;
		 diag.Height = 600;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改指标结果模板";
			 diag.URL = '<%=basePath%>indexResultModel/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 600;
		...
		function del(id,msg){
			bootbox.confirm("确定要该指标模板吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>indexResultModel/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>indexResultModel/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
