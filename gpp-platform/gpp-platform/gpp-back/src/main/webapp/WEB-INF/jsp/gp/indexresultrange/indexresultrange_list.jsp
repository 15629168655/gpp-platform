		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="indexResultRange/list.do?INDEX_ID=${pd.INDEX_ID }" method="post" name="fatForm" id="fatForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">年龄限制</th>
									<th class="center">年龄上限</th>
									<th class="center">年龄下限</th>
									<th class="center">适合性别</th>	
									<th class="center">结果值上限</th>
									<th class="center">结果值下限</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">
								                <c:forEach items="${ISLIMITAGE}" var="s">
								                	<c:if test="${s.key == element.IS_LIMIT_AGE}">${s.value}</c:if>
								                </c:forEach>
											</td>
											<td class="center">${element.AGE_UP }</td>
											<td class="center">${element.AGE_DOWN }</td>
											<td class="center">
...
<script type="text/javascript" src="static/js/jquery.tips.js"></script>
	...
	<script type="text/javascript">
	  ...
	  function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="指标结果范围新增";
		 diag.URL = '<%=basePath%>indexResultRange/add.do?INDEX_ID=${pd.INDEX_ID}';
		 diag.Width = 800;
		 diag.Height = 500;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改指标结果范围";
			 diag.URL = '<%=basePath%>indexResultRange/update.do?ID='+id;
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function del(id,msg){
			bootbox.confirm("确定要该指标模板吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>indexResultRange/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>indexResultRange/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
