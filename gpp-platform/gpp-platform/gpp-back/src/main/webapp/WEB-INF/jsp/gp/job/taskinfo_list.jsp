			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="taskinfo/list.do" method="post" name="userForm" id="userForm">
						<!-- 检索  -->
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">定时任务class</th>
									<th class="center">是否开启</th>
									<th class="center"><i class="ace-icon fa fa-clock-o bigger-150 hidden-480"></i>执行频率</th>
									<th class="center"><i class="ace-icon fa fa-clock-o bigger-150 hidden-480"></i>定时任务描述</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList }" var="varList" varStatus="vs">
										<tr>
											...
											<td class="center">${varList.JOBNAME}</td>
											 <td class="center" id="JOBCLASS">${varList.JOBCLASS}</td>
											 <c:choose>
											 <c:when test="${varList.ZT == 1}">
                                               <td class="center" id="ZT">开启</td>
											</c:when>
											 <c:otherwise>
                                               <td class="center" id="ZT">禁用</td>
											 </c:otherwise>
											  </c:choose>
                                            <td class="center" id="TIMEDES">${varList.TIMEDES}</td>
                                            <td class="center" id="DES">${varList.DES}</td>
                                            ...
                                            <td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${varList.GUID}','${varList.JOBNAME}');">
														删除
													</a>
													</c:if>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${varList.GUID }','${varList.JOBNAME}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增定时任务";
			 diag.URL = '<%=basePath%>taskinfo/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 650;
	　
		...
		function editUser(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改定时任务信息";
			 diag.URL = '<%=basePath%>taskinfo/goEdit.do?ID='+id;
			 diag.Width = 700;
			 diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id,msg){
			bootbox.confirm("确定要删除["+msg+"]的随访记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>taskinfo/del.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>taskinfo/deleteAll.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
								cache: false,
