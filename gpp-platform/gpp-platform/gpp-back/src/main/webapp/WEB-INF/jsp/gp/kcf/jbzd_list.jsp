			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="kcf/jbzd_list.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr JBBM="${var.JBBM}" JBMC="${var.JBMC}" ID="${var.DISEASECODE_ID}">
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.JBBM}</td>
											<td class='center'>
                                                <c:choose>
                                                    <c:when test="${pd.showType == 'diag'}">
                                                        <a onclick="diseaseSelected('${var.JBBM}','${var.JBMC}','${var.DISEASECODE_ID }');" style="cursor: pointer;">${var.JBMC}</a>
                                                    </c:when>
		...
		$(function () {
	     ...
	     // 选择行变色 
	        $("#simple-table > tbody > tr").click(function (){ 
	            ...
	            $("#JBBM").val($(trThis).attr("JBBM"));
	            $("#JBMC").val($(trThis).attr("JBMC"));
	            $("#ID").val($(trThis).attr("ID"));
	        });
	    }) 
	</script>
