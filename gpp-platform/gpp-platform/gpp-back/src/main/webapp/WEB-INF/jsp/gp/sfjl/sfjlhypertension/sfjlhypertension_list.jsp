			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="sfjlhypertension/list.do" method="post" name="userForm" id="userForm">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList }" var="varList" varStatus="vs">
										<tr>
											...
											<td class="center">${varList.HDSB040201_006}</td>
											<td class="center">${varList.USER_NAME}</td>
											<td class="center">${varList.ID_NUMBER}</td>
											<td class="center"><fmt:formatDate pattern="yyyy-MM-dd"  value="${varList.HDSB040201_003 }"/></td>
											<td class="center"><fmt:formatDate pattern="yyyy-MM-dd"  value="${varList.HDSB040201_004 }"/></td>
                                            <td class='center'>
                                                <c:forEach items="${pd.EnumSfjlSFFS}" var="s">
                                                    <c:if test="${s.key == varList.HDSB04020101_004}">${s.value}</c:if>
                                                </c:forEach>
                                            </td>
                                            <td class="center" id="HDSB04020101_005">${varList.HDSB04020101_005}</td>
                                            <td class="center" id="HDSB04020101_033">${varList.HDSB04020101_033}</td>
                                            <td class="center" id="HDSB04020101_006">${varList.HDSB04020101_006}</td>
                                            <td class="center" id="HDSB04020101_008">${varList.HDSB04020101_008}</td>
                                            <td class="center">${varList.HDSB04020101_011}</td>
                                               <td class='center'>
											...
											<td class="center" id="HDSB04020101_032">${varList.HDSB04020101_032}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${varList.GUID }','${varList.HDSB040201_006}');">
														删除
													</a>
													</c:if>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${varList.GUID }','${varList.HDSB040201_006}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增随访记录信息";
			 diag.URL = '<%=basePath%>sfjlhypertension/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 650;
	　
		...
		function editUser(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改随访记录信息";
			 diag.URL = '<%=basePath%>sfjlhypertension/goEdit.do?ID='+id;
			 diag.Width = 700;
			 diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id,msg){
			bootbox.confirm("确定要删除["+msg+"]的随访记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>sfjlhypertension/del.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>sfjlhypertension/deleteAll.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
