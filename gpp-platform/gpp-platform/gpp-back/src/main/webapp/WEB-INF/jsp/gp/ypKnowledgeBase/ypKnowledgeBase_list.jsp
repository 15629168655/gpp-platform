			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="ypKnowledgeBase/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<input class="nav-search-input" autocomplete="off" id="keywords" type="text" name="keywords" value="${pd.keywords }" placeholder="根据药品类型搜索" style="width: 200px"/>
									</div>
								</td>
								<td style="vertical-align:top;padding-left:2px;"> 
									<select class="form-control" id="IS_RECOMMEND" name="IS_RECOMMEND" style="width:120px;">
								...
								<c:when test="${not empty list}">
									<c:forEach items="${list }" var="list" varStatus="vs">
										<tr>
											...
											<td class="center">${list.YP_TYPE_MC}</td>
											<td class="center">
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
                                                       <a class="btn btn-xs btn-success" title="编辑" onclick="edit('${list.ID}');">
                                                         ...
                                                         <a class="btn btn-xs btn-danger" title="删除" onclick="del('${list.ID}');">
                                                           	删除
                                                        </a>
												</div>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增药品信息";
			 diag.URL = '<%=basePath%>ypKnowledgeBase/add.do';
			 diag.Width = 800;
			 diag.Height = 550;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改药品信息";
			 diag.URL = '<%=basePath%>ypKnowledgeBase/update.do?ID='+id;
			 diag.Width = 800;
			 diag.Height = 550;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>ypKnowledgeBase/del.do?ID="+id;
					$.get(url,
						function(data){
							nextPage(${page.currentPage});
						});
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ypKnowledgeBase/deleteAll.do',
						    	data: {ids:str},
								dataType:'json',
