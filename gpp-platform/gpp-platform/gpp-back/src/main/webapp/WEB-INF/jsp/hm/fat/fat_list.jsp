		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="fat/list.do?USER_AGENT_ID=${pd.USER_AGENT_ID }" method="post" name="fatForm" id="fatForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">总胆固醇(mmol/L)</th>
									<th class="center">甘油三酯(mmol/L)</th>
									<th class="center">低密度脂蛋白(mmol/L)</th>
									<th class="center">高密度脂蛋白(mmol/L)</th>	
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="element" varStatus="vs">
										<tr>
											...
											<td class="center">${element.CHO }</td>
											<td class="center">
								                ${element.GLY } 
											</td>
											<td class="center">${element.LOW_FAT  }</td>
											<td class="center">${element.HIG_FAT  }</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		//新增
		  function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增血脂四项记录";
			 diag.URL = '<%=basePath%>fat/add.do?USER_AGENT_ID=${pd.USER_AGENT_ID }';
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改血脂四项记录";
			 diag.URL = '<%=basePath%>fat/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function del(id,msg){
			bootbox.confirm("确定要删除此体型记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>fat/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>fat/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
