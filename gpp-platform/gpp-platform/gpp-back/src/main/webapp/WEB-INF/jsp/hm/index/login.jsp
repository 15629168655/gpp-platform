...
<body style="background-color: azure;">
	<div style="width:100%;text-align: center;margin: 0 auto;position: absolute;">
		<div id="loginbox">
			<form action="" method="post" name="loginForm"
				...
				<div class="control-group">
					<div class="controls" id = "controls">
						<div class="main_input_box" id="controller">
							<span class="add-on bg_lg">
							<i><img height="37" src="static/login/user.png" /></i>
							</span><input type="text" name="loginname" id="loginname" value=""placeholder="请输入用户名" />
								<div id="list" style="background-color:white; display:block; margin-top: -2px;position:absolute; text-align: left;" >
			...
			<div class="controls">
				<div class="main_input_box">
					<font color="white"><span id="nameerr">Copyright © 2015-2017 武汉市同步远方信息技术开发有限公司</span></font>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
        ...
        function changeBackColor_over(div){
         $(div).css("background-color","#CC00FF");
        }
        //鼠标离开内容
        ...
        function setSearch_onclick(div){
         $("#loginname").val($(div).children("input[type='hidden']").attr("value"));
		...
		//	}
				...
				var widthdiv = $("#controls").width();
				var val =  (widthdiv - widthInput)/2;
				val += 20;
				$("#list").css({"margin-left": val});
				$("#list").width(widthInput+11); 
				...
				success:function(map){
					if(map){
					    ...
					    for(var i=0;i<length;i++){
							...
							if(lengthOfString > textLength+1) {
								org_name = org_name.substring(0, org_name.length - (lengthOfString - textLength)) + "...";
							}
					     //每一个div还有鼠标移出、移入点击事件
					     html+="<div onclick='setSearch_onclick(this)' onmouseout='changeBackColor_out(this)' onmouseover='changeBackColor_over(this)'><input type='hidden' value='"+map.varList[i].USERNAME+"'/>"+"<lable style='folat: left; font-weight:bolder;'>"+map.varList[i].USERNAME+"     </lable>"+"<lable style='folat: right;'>"+map.varList[i].PROVIDER_NAME+"("+ map.varList[i].PROVIDER_CODE +")-"+org_name+"</lable>"+"</div>";
					    }
					    if(maxlength  == length) {
					    	 html += "<div>....</div>"	
					    }
