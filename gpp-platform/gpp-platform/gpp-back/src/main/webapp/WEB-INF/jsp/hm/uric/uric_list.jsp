		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					<div class="col-xs-12">
						<!-- 检索 -->
						<form action="uric/list.do?USER_AGENT_ID=${pd.USER_AGENT_ID }" method="post" name="uricForm" id="uricForm">
	...
	<script type="text/javascript">
		...
		//新增
		  function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增尿酸记录";
			 diag.URL = '<%=basePath%>uric/add.do?USER_AGENT_ID=${pd.USER_AGENT_ID }';
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改尿酸记录";
			 diag.URL = '<%=basePath%>uric/update.do?ID='+id;
			 diag.Width = 900;
			 diag.Height = 1000;
		...
		//删除
		function del(id,msg){
			bootbox.confirm("确定要删除此尿酸记录吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>uric/delete.do?ID="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>uric/deleteAll.do?tm='+new Date().getTime(),
						    	data: {IDS:str},
								dataType:'json',
