...
<body class="no-skin">
							...
							<c:when test="${not empty varList}">
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										...
										<td class='center'>
                                                <c:forEach items="${pd.EnumNotice}" var="s">
                                                    <c:if test="${s.key == var.TYPE}">${s.value}</c:if>
                                                </c:forEach>
                                            </td>
										<td class="center" style="WORD-WRAP: break-word;">${var.BT}</td>
										<td class="center" style="WORD-WRAP: break-word;">${var.REMARK}</td>
										<td class='center'>
										...
										<td class="center">
											<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${var.CREATETIME}"/>
										</td>
										<td class="center">
											<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${var.UPDATETIME}"/>
										</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>notice/add.do';
			 diag.Width = 800;
			 diag.Height = 490;
		...
		function edit(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>notice/update.do?ID='+ID;
			 diag.Width = 800;
			 diag.Height = 495;
			 diag.CancelEvent = function(){ //关闭事件
