		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<form action="district/${msg }.do" name="Form" id="Form" method="post">
						<input type="hidden" name="REGICODE" id="REGICODE" value="${pd.REGICODE}"/>
						<!-- 级次 -->
						<input type="hidden" name="FINANCELEVEL" id="FINANCELEVEL" value="${pds.FINANCELEVEL}">
						<%-- <input type="hidden" name="P_REGICODE" id="P_REGICODE" value="${null == pd.P_REGICODE ? REGICODE:pd.P_REGICODE}"/> --%>
						<input type="hidden" name="P_REGICODE" id="P_REGICODE" value="${null ==pds.REGICODE?'AAA':pds.REGICODE }">
						<div id="zhongxin">
						<table id="table_report" class="table table-striped table-bordered table-hover">
							<tr>
								...
								<td>
									<div class="col-xs-4 label label-lg label-light arrowed-in arrowed-right">
										<b>${null == pds.REGINAME ?'(无) 此项为顶级':pds.REGINAME}</b>
									</div>
								</td>
							</tr>
							<tr>
								<td style="width:120px;text-align: right;padding-top: 13px;">行政区划编码:</td>
							...
							<tr>
								<td style="width:120px;text-align: right;padding-top: 13px;">行政区划名称:</td>
								<td><input type="text" name="REGINAME" id="REGINAME" value="${pd.REGINAME}" maxlength="50" placeholder="这里输入名称" title="名称" style="width:98%;"/></td>
							</tr>
							<%-- <tr>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		//判断编码是否存在
		function hasBianma(){
			var REGICODE = $.trim($("#bianma").val());
			if("" == REGICODE)return;
			$.ajax({
				type: "POST",
				url: '<%=basePath%>district/hasBianma.do',
		    	data: {REGICODE:REGICODE,tm:new Date().getTime()},
				dataType:'json',
				cache: false,
