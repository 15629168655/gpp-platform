...
public class AppUtil  {
	...
	public static boolean checkParam(String method, PageData pd){
		...
		}else if(method=="loginByPhoneCode"){//用户根据手机号+验证码登陆
			paramArray = Const.APP_LOGINBYPHONECODE_PARAM_ARRAY;  
			valueArray = Const.APP_LOGINBYPHONECODE_VALUE_ARRAY;
		...
		}else if(method=="validateTeamNameApp"){//签约团队名称唯一性判断
			paramArray = Const.APP_ADDGPP_TEAM_VALIDATE_PARAM_ARRAY;
			valueArray = Const.APP_ADDGPP_TEAM_VALIDATE_VALUE_ARRAY;
		}else if(method=="validateMemberApp"){//签约团队成员唯一性判断
			paramArray = Const.APP_ADDGPP_TEAMMEMBER_VALIDATE_PARAM_ARRAY;
			valueArray = Const.APP_ADDGPP_TEAMMEMBER_VALIDATE_VALUE_ARRAY;
		...
		}else if(method=="TeamQueryPageByProvider"){//团队查询详情
			paramArray = Const.APP_TEAMQUERYPAGEBYPROVIDER_AGREEMENT_PARAM_ARRAY;
			valueArray = Const.APP_TEAMQUERYPAGEBYPROVIDER_AGREEMENT_VALUE_ARRAY;
		...
		}else if(method=="UserPhoneBind"){ //医生用户绑定手机号
			paramArray = Const.APP_USER_PHONEBIND_PARAM_ARRAY;  //参数
			valueArray = Const.APP_USER_PHONEBIND_VALUE_ARRAY;  //参数名称
		...
		}else if(method=="appEditSfTemp"){//随访模版修改
			paramArray = Const.APP_SFTEMP_EDIT_PARAM_ARRAY;
			valueArray = Const.APP_SFTEMP_EDIT_VALUE_ARRAY;
		}else if(method=="appAddXDT"){//心电图记录新增
			paramArray = Const.APP_XDT_ADD_PARAM_ARRAY;
			valueArray = Const.APP_XDT_ADD_VALUE_ARRAY;
		}
		...
		}else if(method=="getProviderByUserAndCode"){//根据医生机构查询医护人员
			paramArray = Const.APP_GETPROVIDERBYUSERANDCODE_PARAM_ARRAY;
			valueArray = Const.APP_GETPROVIDERBYUSERANDCODE_VALUE_ARRAY;
		...
		else if(method == "initJkgl") {//app健康管理初始化
			paramArray = Const.APP_JKGL_INIT_PARAM_ARRY;
			valueArray = Const.APP_JKGL_INIT_VALUE_ARRY;
		}
		...
		else if(method == "saveDeviceInformation") {//打开app保存设备消息
			paramArray = Const.APP_DEVICEPUSH_SAVEDEVICE_PARAM_ARRY;
			valueArray = Const.APP_DEVICEPUSH_SAVEDEVICE_VALUE_ARRY;
		}
		else if(method == "saveUserInformation") {//登录app获得用户信息
			paramArray = Const.APP_DEVICEPUSH_SAVEUSER_PARAM_ARRY;
			valueArray = Const.APP_DEVICEPUSH_SAVEUSER_VALUE_ARRY;
		}
		else if(method == "editFourMsgState") {//登录进来保存用户设置（站内信，官博）的状态
			paramArray = Const.APP_DEVICEPUSH_SAVEFOURMSGTYPE_PARAM_ARRY;
			valueArray = Const.APP_DEVICEPUSH_SAVEFOURMSGTYPE_VALUE_ARRY;
		}
